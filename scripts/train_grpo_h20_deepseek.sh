#!/bin/bash
# VRAG-RL GRPO 训练脚本 — DeepSeek API 版本
# 使用 DeepSeek 替代 DashScope 作为 reward 模型
#
# 使用方法：
#   export DEEPSEEK_API_KEY=your_key
#   bash train_grpo_h20_deepseek.sh
...
if [ ! -f "${train_data}" ]; then
    ...
    echo "        请先运行: python scripts/prepare_data.py --mode slidevqa"
    exit 1
fi
...
# ── 日志 / 实验名 ─────────────────────────────────────
project_name="vrag_finance_deepseek"
experiment_name="h20_single_grpo_deepseek_$(date +%Y%m%d_%H%M)"

echo "[INFO] 实验名: ${experiment_name}"
echo "[INFO] 训练数据: ${train_data}"
echo "[INFO] Reward 模型: DeepSeek V3 (${rm_url})"
...
python -m verl.trainer.main_ppo \
    ...
    reward_model.rm_api_key="${rm_api_key}" \
